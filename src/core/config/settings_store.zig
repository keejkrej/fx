...
pub const Store = struct {
    ...
    pub fn initFromHome(alloc: Allocator, home_path: []const u8, mode: OpenMode) !Store {
        ...
        if (mode == .writable) {
            durable_home.setPermissions(zio, io_mod.permissionsFromUnixMode(0o700)) catch {
                return error.PrivateStatePermissionsUnsupported;
            };
        }
        const stat = try durable_home.stat(zio);
        if (stat.kind != .directory) return error.DurablePathUnsafe;
        const durable_mode = io_mod.posixMode(stat.permissions) & 0o777;
    ...
    fn loadRawPrimary(self: *Store, alloc: Allocator) !RawPrimary {
        ...
        if (self.mode == .writable) {
            file.setPermissions(zio, io_mod.permissionsFromUnixMode(0o600)) catch {
                return error.PrivateStatePermissionsUnsupported;
            };
        }
        const verified_stat = if (self.mode == .writable) try file.stat(zio) else stat;
        const primary_mode = io_mod.posixMode(verified_stat.permissions) & 0o777;
...
test "user patch snapshots and removes legacy workspace copies" {
    ...
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o600), io_mod.posixMode(recovery_stat.permissions) & 0o777);
...
test "missing user settings is created through private durable commit" {
    ...
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o600), io_mod.posixMode(stat.permissions) & 0o777);
}
...
test "read only settings rejects group or world writable policy files" {
    ...
    root_dir.setPermissions(io_mod.getIo(), io_mod.permissionsFromUnixMode(0o777)) catch return error.SkipZigTest;
    ...
    root_dir.setPermissions(io_mod.getIo(), io_mod.permissionsFromUnixMode(0o755)) catch return error.SkipZigTest;
    var file = try root_dir.openFile(io_mod.getIo(), "settings.json", .{ .mode = .read_write });
    file.setPermissions(io_mod.getIo(), io_mod.permissionsFromUnixMode(0o666)) catch {
