...
pub fn parse(value: []const u8) ?ProviderId {
    ...
    if (std.ascii.eqlIgnoreCase(value, "openai_compatible") or
        std.ascii.eqlIgnoreCase(value, "openai-compatible"))
...
test "explicit providers authorize only their own credential origins" {
    ...
    try std.testing.expect(!authorizesCredential(.openai_compatible, .ai_gateway_api_key));
}
