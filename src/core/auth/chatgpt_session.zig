...
fn openExistingPrivateFxDir(home_dir: *io_mod.VerifiedDir) !io_mod.VerifiedDir {
    ...
    if (io_mod.posixMode(initial_stat.permissions) & 0o200 == 0) return error.PrivateStatePermissionsUnsupported;
    dir.setPermissions(io_mod.getIo(), io_mod.permissionsFromUnixMode(0o700)) catch {
        return error.PrivateStatePermissionsUnsupported;
    };
    const stat = try dir.stat(io_mod.getIo());
    if (stat.kind != .directory or !io_mod.matchesUnixMode(stat.permissions, 0o700)) {
        return error.PrivateStatePermissionsUnsupported;
    }
    return .{ .dir = dir };
}
