...
fn ensureManagedDir(parent_path: []const u8, child_name: []const u8) !void {
    ...
    for (0..2) |_| {
        var child = parent.openDir(zio, child_name, .{
            ...
            error.FileNotFound => {
                parent.createDir(zio, child_name, io_mod.permissionsFromUnixMode(0o700)) catch |create_err| switch (create_err) {
        ...
        child.setPermissions(zio, io_mod.permissionsFromUnixMode(0o700)) catch
            return error.CorruptArtifactStore;
        const stat = try child.stat(zio);
        if (stat.kind != .directory or io_mod.posixMode(stat.permissions) & 0o777 != 0o700) {
            return error.CorruptArtifactStore;
        }
        return;
    }
...
test "web_fetch artifact store creates private managed directories" {
    ...
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o700), io_mod.posixMode(artifacts_stat.permissions) & 0o777);
    try std.testing.expectEqual(@as(std.posix.mode_t, 0o700), io_mod.posixMode(web_fetch_stat.permissions) & 0o777);
}
