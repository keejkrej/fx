...
fn openUsageRecoveryProfileRoot(
    ...
    if (stat.kind != .directory or
        !io_mod.matchesUnixMode(stat.permissions, 0o700))
    {
...
fn prepareWritableSessionDir(dir: std.Io.Dir) !void {
    const permissions = io_mod.permissionsFromUnixMode(0o700);
...
test "doctor reports unsafe managed child artifacts" {
    ...
    try managed_dir.setPermissions(io_mod.getIo(), io_mod.permissionsFromUnixMode(0o755));
